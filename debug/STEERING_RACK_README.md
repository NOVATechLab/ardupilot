...
| `JRVS_RACK_OL`    | 0    | Діагностичний open-loop режим (0=вимк, 1=увімк) |

> Зміна параметрів через GCS **не потребує перезбірки** firmware.

### Діагностика: `JRVS_RACK_OL` (open-loop)

Коли `JRVS_RACK_OL=1`, скрипт повністю ігнорує потенціометр і керує реле напряму
від PWM (просте bang-bang: ліво/центр/право за позицією стіка, без цілі по
напрузі і без апаратних лімітів). Це дозволяє відділити проблему потенціометра
від проблеми реле/плати/мотора:

- Рейка **рухається** в OL-режимі → проблема в потенціометрі або проводці до
  нього (сама силова частина — реле, проводка, мотор — справна).
- Рейка **не рухається** і в OL-режимі → потенціометр ні до чого, шукайте
  проблему в реле / проводці до мотора / самому моторі.

> ⚠️ В `JRVS_RACK_OL=1` немає жодного захисту від апаратних меж (`HW_GUARD`) —
> використовувати тільки короткочасно під наглядом, вручну зупиняючи стік у
> центрі. Не залишайте увімкненим для звичайної їзди.
